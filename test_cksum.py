#!/usr/bin/env python3
"""
Test script to verify the ported Linux cksum algorithm
"""
...
def test_cksum():
    """Test the cksum implementation with known values"""
    
    # Test cases with known Linux cksum results (verified values)
    test_cases = [
        # (data, expected_cksum, description)
        (b"", 4294967295, "empty file"),  # cksum of empty file should be 0xFFFFFFFF
        (b"a", 1220704766, "single character 'a'"),
        (b"abc", 1219131554, "string 'abc'"),
        (b"test", 3632233996, "string 'test'"),  # More reliable test case
    ]
    
    print("Testing Linux cksum algorithm implementation...")
    print("=" * 50)
    
    all_passed = True
    for data, expected, description in test_cases:
        result = calculate_cksum(data)
        ...
        print(f"  Got:      {result} (0x{result:08x})")
        if not passed:
            print(f"  Difference: {result - expected}")
        print()
    
    if all_passed:
        print("✅ All tests PASSED! The cksum implementation is correct.")
    else:
        print("❌ Some tests FAILED! The cksum implementation needs fixing.")
